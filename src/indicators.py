...
def check_market_cap_krw(info: dict, min_krw: float) -> bool:
    """시가총액(원) 최소값 확인"""
    cap = info.get("market_cap", 0) or 0
    return cap >= min_krw


def check_price_change_rate(info: dict, min_rate: float) -> bool:
    """전일 대비 등락률 확인 (%)"""
    rate = info.get("change_rate", 0) or 0
    return rate >= min_rate


def check_upper_limit_proximity(info: dict, within_pct: float) -> bool:
    """상한가(+30%) 근접 N% 이내"""
    close = info.get("close", 0)
    if not close:
        return False
    upper_limit = close * 1.30
    gap_pct = (upper_limit - close) / close * 100
    return gap_pct <= within_pct


def check_market_filter(info: dict, market: str) -> bool:
    """KOSPI / KOSDAQ 구분 필터"""
    return info.get("market", "") == market
