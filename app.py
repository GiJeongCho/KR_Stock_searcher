...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "kr-stock-screener-secret-key-change-me")
...
@login_required
def add_watchlist():
    ticker = (request.get_json() or {}).get("ticker", "").strip()
    if not ticker:
...
def del_watchlist(ticker):
    ...
    try:
        user = s.get(User, current_user.id)
        wl = [t for t in user.get_watchlist() if t != ticker]
        user.set_watchlist(wl)
        s.commit()
        return jsonify({"ok": True, "watchlist": wl})
    finally:
...
@app.route("/api/ticker_names", methods=["POST"])
def ticker_names():
...
if __name__ == "__main__":
    ...
    if "--reload" in sys.argv:
        app.run(debug=True, use_reloader=True, host="0.0.0.0", port=4999, threaded=True)
    else:
        from waitress import serve
        start_background_scanner()
        print("Waitress 서버 시작: http://0.0.0.0:4999")
        serve(app, host="0.0.0.0", port=4999, threads=8)
