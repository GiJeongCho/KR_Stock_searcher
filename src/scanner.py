...
API 호출 전략:
  Stage 1 - 전종목 그대로 통과 (선필터 없음)
  Stage 2 - 전종목 × 타임프레임별 키움 분봉 개별 조회 (ThreadPoolExecutor 5)
...
def _preload_timeframes(
    ...
    """종목을 타임프레임별로 개별 조회 → fetcher 캐시에 저장"""
    tasks = [(t["code"], iv) for t in candidates for iv in intervals]
    total = len(tasks)
    if total == 0:
        return

    _put(q, {"type": "status", "msg": f"[1단계] {total:,}건 분봉 로드 시작..."})
    loaded = 0
